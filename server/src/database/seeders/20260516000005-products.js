...
module.exports = {
  async up(queryInterface) {
    ...
    await queryInterface.bulkInsert('products', [
      makeProduct(P.p1,  'samsung-55-qled-4k-smart-tv',       'SAM-TV-55Q',   'Samsung 55" QLED 4K Smart TV',                'Samsung',  'Stunning 4K QLED display with Quantum HDR and built-in streaming apps.',        c('tvs'),                 59999, 79999,  42, 4.5, 1243,  { isOnOffer: true }),
      ...
      makeProduct(P.p3,  'sony-wh-1000xm5-wireless',           'SNY-WH-XM5',   'Sony WH-1000XM5 Wireless Headphones',       'Sony',     'Industry-leading noise cancellation with 30-hour battery life.',                 c('headphones-audio'),   29900, 37900,  88, 4.7, 2100,  { isOnOffer: true }),
      makeProduct(P.p4,  'playstation-5-console-disc',         'PS5-DISC-001', 'PlayStation 5 Console (Disc Edition)',        'Sony',     'Next-gen gaming with ultra-high speed SSD and ray tracing support.',             c('gaming'),             44999, null,    0, 4.8, 12450, { reserveAvailable: true }),
      ...
      makeProduct(P.p11, 'apple-macbook-air-m3-15',            'APP-MBA15M3',  'Apple MacBook Air 15" M3',                 'Apple',    'Thin and light with the powerful M3 chip and 18-hour battery.',                 c('computers-tablets'), 149900, null,  55, 4.8, 3200),
      ...
      makeProduct(P.p13, 'canon-eos-r6-mark-ii',               'CAN-R6M2-01',  'Canon EOS R6 Mark II Mirrorless',           'Canon',    'Full-frame 24.2MP, 40fps burst, and advanced autofocus.',                         c('cameras'),           249900, null,   12, 4.9, 340),
      makeProduct(P.p14, 'gopro-hero-12-black',                'GOP-H12B-01',  'GoPro HERO12 Black',                        'GoPro',    '5.3K video, HyperSmooth 6.0, and waterproof to 33ft.',                           c('cameras'),           44900, 49900,  78, 4.6, 890,  { isOnOffer: true }),
      makeProduct(P.p15, 'philips-hue-white-colour-starter',   'PHL-HUE-WCS',  'Philips Hue White & Colour Starter Kit',    'Philips',  '3 bulbs, bridge, and 16 million colours with app control.',                      c('smart-home'),         12999, 17999,  110, 4.5, 560,  { isOnOffer: true }),
    ...
    // ── Home & Garden ────────────────────────────────────────────────────────
    await queryInterface.bulkInsert('products', [
      makeProduct(P.p17, 'dyson-v15-detect-absolute',          'DYS-V15DA-01', 'Dyson V15 Detect Absolute Cordless Vacuum',   'Dyson',    'Laser illumination reveals microscopic dust. Up to 60 minutes runtime.',           c('diy-tools'),          54999, 64999,  28, 4.7, 3210, { isOnOffer: true }),
      makeProduct(P.p18, 'ninja-foodi-max-9in1',              'NJ-FMAX9IN1',  'Ninja Foodi MAX 9-in-1 Multi-Cooker',         'Ninja',    'Pressure cook, air fry, slow cook, bake, and more in one pot.',                    c('kitchen-dining'),     17999, 24999,  95, 4.6, 4560, { isOnOffer: true }),
      ...
      makeProduct(P.p21, 'john-lewis-anyday-sofa-bed',         'JL-ANYSFB-01', 'John Lewis Anyday Sofa Bed',                'John Lewis','Compact sofa that folds out into a comfortable double bed.',                        c('furniture'),          39900, 49900,  18, 4.2, 340,  { isOnOffer: true }),
      ...
      makeProduct(P.p23, 'weber-spirit-ii-e-310-bbq',          'WB-SPII-E310', 'Weber Spirit II E-310 Gas BBQ',             'Weber',    '3 burners, GS4 grilling system, and 10-year warranty.',                          c('garden-outdoor'),     64900, null,   22, 4.8, 450),
      makeProduct(P.p24, 'philips-hue-outdoor-light-strip',    'PHL-HUE-OLS',  'Philips Hue Outdoor Light Strip',           'Philips',  'Weatherproof LED strip with millions of colours and app control.',                c('garden-outdoor'),     14999, 19999,  40, 4.4, 320,  { isOnOffer: true }),
    ...
    // ── Health & Beauty ──────────────────────────────────────────────────────
    await queryInterface.bulkInsert('products', [
      makeProduct(P.p27, 'dyson-supersonic-hair-dryer',        'DYS-SUP-HD',   'Dyson Supersonic Hair Dryer',               'Dyson',    'Fast drying with intelligent heat control and magnetic attachments.',              c('haircare'),          32900, null,   60, 4.7, 4100),
    ...
    // ── Sports & Leisure ─────────────────────────────────────────────────────
    await queryInterface.bulkInsert('products', [
      makeProduct(P.p35, 'fitbit-charge-6-fitness-tracker',    'FTB-C6-001',   'Fitbit Charge 6 Fitness Tracker',           'Fitbit',   'Built-in GPS, 40+ exercise modes, and 7-day battery life.',                      c('fitness-equipment'), 12999, 15999,  150, 4.4, 2340, { isOnOffer: true }),
    ...
    // ── Toys & Games ─────────────────────────────────────────────────────────
    await queryInterface.bulkInsert('products', [
      makeProduct(P.p41, 'lego-technic-ferrari-daytona-sp3',   'LEG-42143',    'LEGO Technic Ferrari Daytona SP3',          'LEGO',     '3778 pieces, V12 engine, and opening doors. A collector\'s masterpiece.',         c('action-figures'),    34999, 39999,  35, 4.9, 892,  { isOnOffer: true }),
      makeProduct(P.p42, 'barbie-dreamhouse-playset',          'BRB-DREAM-01', 'Barbie DreamHouse Playset',                 'Barbie',   '3 stories, 8 rooms, pool, slide, and 70+ accessories.',                            c('action-figures'),    9999,  null,   60, 4.6, 1200),
